...
const ExamBlog = () => {
  ...
  useEffect(()=>{
    setLoading(true)
    getPosts({type: "exam", setLoading, setPosts})
  }, [setPosts])
  if(loading){
    return <Loading/>
  }
  ...
  return (
    <div className="flex flex-wrap w-full justify-center gap-3 px-[8%] ">
       {posts.map(({title,featuredImageUrl,excerpt, slug}, i)=>{
                return(
                    <Link href={`/exam/${slug}`} key={i} className='bg-primary flex flex-col h-full w-[90%] md:w-[31%] rounded-xl shadow-md shadow-current'>
